...
Build and run kongfig in a container with persistent config.
This also serves as a simple example of using Docker — building images,
running containers, mounting volumes, and cleaning up.

```sh
make docker-build   # build the image
make docker-run     # start container in background
make docker-exec    # shell into the running container
make docker-stop    # stop and remove container + image
make docker-demo    # round-trip demo: config survives container removal
```

The container mounts `~/.kongfig` from the host, so settings persist
...
| k5f2.h               | 📑 | Libk5f2                                    |
| k5f2_impl.c          | 📄 | k5f2.h single-header implementation unit   |
| kongfig              | 🔩 | Shell Version                              |
| kongfig.1            | 📄 | Man Page                                   |
| kongfig.py           | 🐍 | Python script                              |
